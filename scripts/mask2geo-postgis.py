...
from webclient.models import CategoryLabel, CategoryType, Color, Image, ImageFilter, ImageLabel, ImageSourceType, \
    ImageWindow, Labeler, Tile, TileSet, TiledGISLabel
...
for i in range(0, len(p)):
    ...
    try:
        ...
        print(type(poly))
        geom = GEOSGeometry(geojson.dumps(poly))
        ...
        label_type = "P"
        tl = TiledGISLabel(geometry=geom, label_json=geojson.loads(label_json), label_type=label_type,
                           northeast_Lat=northeast_Lat, northeast_Lng=northeast_Lng, southwest_Lat=southwest_Lat,
                           southwest_Lng=southwest_Lng, zoom_level=zoom_level, category=category)
        tl.save()
